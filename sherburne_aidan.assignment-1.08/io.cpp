...
uint32_t io_teleport_pc(dungeon *d) {
  ...
  while ((c = getch()) != 'g' && c != '.' && c != 'r') {
    if (charpair(dest)) {
      ...
      mvaddch(dest[dim_y] + 1, dest[dim_x],
              character_get_symbol(charpair(dest)));
...
static void io_list_monsters_display(dungeon *d, character **c,
  ...
  for (i = 0; i < count; i++) {
    snprintf(s[i], 40, "%16s%c: %2d %s by %2d %s",
             ...
             c[i]->symbol, abs(c[i]->position[dim_y] - d->PC->position[dim_y]),
             ((c[i]->position[dim_y] - d->PC->position[dim_y]) <= 0 ? "North"
                                                                    : "South"),
             abs(c[i]->position[dim_x] - d->PC->position[dim_x]),
             ((c[i]->position[dim_x] - d->PC->position[dim_x]) <= 0 ? "West"
                                                                    : "East"));
    if (count <= 13) {
